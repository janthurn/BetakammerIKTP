...
double DipTiefe(TH1* hin,TString canvasName, bool draw){
    ...
    double depth = 2*average_Dipp/(average_BeforeDipp+averageAfterDipp);
...
TH1D*  ExtractSiliconEnergyXCutYCut(TString filename,double Limit1, bool outside){
    ...
    for (int i=0; i<entries; i++)    {
            ...
            if(i %100000 ==0)std::cout << "entry="<<i << "\n";
    }
...
TH1D*  ExtractSiliconEnergyZCut(TString filename,double Limit1, bool below){

    TH1D* hOut;
    TH1D* energy_below= new TH1D(Form("energy_below%s",filename),";energy(keV) ",200,0,2000);
    TH1D* energy_above= new TH1D(Form("energy_above%s",filename),";energy(keV) ",200,0,2000);
    TH1D* ZPosition = new TH1D(Form("ZPosition",filename),"vertex z position (mm)",400,0,40);
    TFile * myfile= new TFile(filename,"READ");
    Double_t Eelectron,mzp;
    TTree* tree= (TTree*)myfile->Get("siliconOutput");
    tree->SetBranchStatus("SiliconEnergy",1);
    tree->SetBranchAddress("SiliconEnergy",&Eelectron);
    TTree* tree2= (TTree*)myfile->Get("MC_Input");
    tree2->SetBranchAddress("mc_zpos",&mzp);
    int entries= tree->GetEntries();
    double Offset = 9.333; //mm
    double Dicke =34e-3; //mm
    for (int i=0; i<entries; i++)    {
            tree->GetEntry(i);
            tree2->GetEntry(i);
            double Position = (mzp-Offset+(Dicke/2))*1e3;
            ZPosition->Fill(Position);
            if(i %25000 ==0)std::cout << "entry="<<i << " Z ="<<Position<<"\n";
            if(Position>=Limit1)energy_above->Fill(Eelectron);
            else if(Position<Limit1)energy_below->Fill(Eelectron);

    }
    if(below) hOut =energy_below;
    else hOut =energy_above;
...
void TestDifferentExtraction5(){
  ...
  gStyle->SetOptStat(0);
    ...
    hNormal->GetXaxis()->SetRangeUser(0,1600);
    ...
    leg1->AddEntry(hOnlysample,"Only Sample","l");
     ...
     double normal = DipTiefe(hNormal,"complete",true);
      double ohne HS  =DipTiefe(hOhneHS,"complete ohne HS",false);
      double onlySample= DipTiefe(hOnlysample,"only",false);
    ...
    //     gDipRatioBefore->Draw("AP");
    //     gDipRatioAfter->Draw("P");

    //==========================================
    double Limit=32;
    TH1D * hOben = ExtractSiliconEnergyZCut("Si31SampleTest3.root",Limit,false);
    TH1D * hUnten = ExtractSiliconEnergyZCut("Si31SampleTest3.root",Limit,true);

    hOben->SetLineColor(kRed);
    hUnten->SetLineColor(kBlue);
    hOben->GetYaxis()->SetRangeUser(0,8000);
    hOben->GetXaxis()->SetRangeUser(0,1600);

    TLegend * leg2 = new TLegend(0.35,0.67,0.9,0.9);
    leg2->SetFillColor(0);
    // leg1->SetBorderSize(0);
    leg2->AddEntry(hOben,Form("Events with Z between %.1f - 34 #mum",Limit),"l");
    leg2->AddEntry(hUnten,Form("Events below %.1f #mum",Limit),"l");
    // leg2->AddEntry(hOnlysample,"Only Sample","l");
    ...
    double tiefeOberhalb = DipTiefe(hOben,"Oberhalb",true);
    double tiefeUnterhalb  =DipTiefe(hUnten,"Unterhalb",true);
